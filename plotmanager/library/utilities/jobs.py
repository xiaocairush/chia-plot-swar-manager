...
def load_jobs(config_jobs):
    ...
    for info in config_jobs:
        ...
        job.contract_address = info.get('contract_address', None)
...
def monitor_jobs_to_start(jobs, running_work, max_concurrent, max_for_phase_1, next_job_work, chia_location,
    ...
    candidate_job = None
    candidate_job_index = 0
    # record latest running_job start_time for every job
    job_start_date_dict = {}
    for pid in running_work:
        running_job = running_work[pid].job
        start_date = running_work[pid].datetime_start
        if running_job.name not in job_start_date_dict or job_start_date_dict[running_job.name] < start_date:
            job_start_date_dict[running_job.name] = start_date
    ...
    for i, job in enumerate(jobs):
        ...
        # if job is not running, just start it
        if job.name not in job_start_date_dict :
            candidate_job = job
            candidate_job_index = i
            break;
        ...
        if candidate_job is None:
            ...
            candidate_job_index = i
            continue
        # keep candidate job as the oldest job
        if job_start_date_dict[candidate_job.name] > job_start_date_dict[job.name] :
            candidate_job = job
            candidate_job_index = i
    ...
    if candidate_job.stagger_minutes:
            next_job_work[candidate_job.name] = datetime.now() + timedelta(minutes=candidate_job.stagger_minutes)
            logging.info(f'Calculating new job stagger time. Next stagger kickoff: {next_job_work[candidate_job.name]}')
    ...
    jobs[candidate_job_index] = deepcopy(job)
    if work is None:
        return jobs, running_work, next_job_work, next_log_check
    total_phase_1_count += 1
