...
def _analyze_log_end_date(file_path, contents):
    ...
    end_date = dateparser.parse(time.ctime(os.path.getmtime(file_path)))
    return dict(
...
def analyze_log_dates(log_directory, analysis):
    ...
    for file_path, contents in files.items():
        data = _analyze_log_end_date(file_path, contents)
        if data is None:
            continue
...
def analyze_log_times(log_directory):
    ...
    for file_path, contents in files.items():
        count += 1
        phase_times = get_phase_info(contents, pretty_print=False)
...
def check_log_progress(jobs, running_work, progress_settings, notification_settings, view_settings,
    ...
    for pid, work in list(running_work.items()):
        ...
        phase_times = get_phase_info(data, view_settings)
        current_phase = 1
        if phase_times:
